"""Spider to scrap KBS news website"""
...
from crwkbs.constant import BASE_URL, SITEMAP_URL

from crwkbs.items import ArticleData
from crwkbs.utils import (
...
class CrwKbs(scrapy.Spider, BaseSpider):
    """Spider class to scrap sitemap and articles of kbs News site"""
    ...
    def parse(self, response: str, **kwargs) -> None:
        ...
        if "dailyNewsList.xml" in response.url:
            for single_date in self.date_range_lst:
                try:
                    self.logger.debug("Parse function called on %s", response.url)
                    yield scrapy.Request(
                        f"https://news.kbs.co.kr/sitemap/dailyNewsList/{single_date}.xml",
    ...
    def parse_sitemap(self, response: str) -> None:
        ...
        try:
            for url, title in zip(
                ...
                .getall(),
                Selector(response, type="xml")
                .xpath("//sitemap:title/text()", namespaces=self.news_namespace)
                .getall(),
            ):
