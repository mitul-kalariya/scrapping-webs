...
from crwmbcnews.test.helpers.utils import get_article_content, online_response_from_url
...
class TestArticle(unittest.TestCase):
    ...
    def test_parse(self):
        for article in TEST_ARTICLES:
            ...
            spider = MbcNewsSpider(type="article", url=article["url"])
    ...
    def _test_parse_json_data_format(self, article, test_article_data):  # noqa: C901
        ...
        if article[0].get("parsed_data").get("text"):
            ...
            with self.subTest():
                self.assertEqual(
                    len(article[0].get("parsed_data").get("text")),
                    1,
                    "format mismatch for parsed_data--> text, more than 1 string found",
                )
        else:
            with self.subTest():
