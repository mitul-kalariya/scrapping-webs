...
from crwsueddeutsche.items import (
    ArticleRawResponse,
    ...
    for block in parsed_data:
        if "NewsArticle" in json.loads(block).get("@type"):
            return {
                "publisher": [
                    {
                        "@id": response.css(".custom-l20qco::attr(href)").get(),
                        "@type": json.loads(block)
                        .get("publisher", {})
                        .get("@type", None),
                        "name": json.loads(block).get("publisher", {}).get("name"),
                        "logo": {
                            "type": json.loads(block)
                            .get("publisher", {})
                            .get("logo", {})
...
def get_thumbnail_image_video(response: str) -> dict:
    ...
    """
    p_tag_image_caption = response.css(".css-cd29nr p::text").getall()
    small_tag_image_caption = response.css(".css-cd29nr small::text").getall()
    image_caption = ["".join(p_tag_image_caption + small_tag_image_caption)]
    return {
        "images": [
            {"link": img, "caption": cap}
            for img, cap in itertools.zip_longest(
                response.css(".css-1b63ry7::attr(src)").getall(),
                image_caption,
                fillvalue=None,
            )
        ],
