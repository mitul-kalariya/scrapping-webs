...
import json5
from crwyonhapnews.items import (
    ArticleRawResponse,
...
def get_parsed_json_filter(blocks: list, misc: list) -> dict:
    ...
    for block in blocks:
        ...
        elif "ImageGallery" in json5.loads(block).get("@type", [{}]) or "ImageObjects" in json5.loads(block).get("@type", [{}]):
...
def get_author_details(parsed_data: list, response: str) -> dict:
    ...
    return {"author": [{"name": response.css("meta[name*='author']::attr(content)").get()}]}
