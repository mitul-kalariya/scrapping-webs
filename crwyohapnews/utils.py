...
def get_country_language_details(response) -> dict:
    ...
    return {"source_country": ["Korea"], "source_language": [mapper.get(language)]}
...
def get_author_details(response: str) -> dict:
    ...
    """
    author_details = response.css('meta[property="dd:author"]::attr(content)').getall()
    return {"author": {"name": ",".join(author_details)}}
...
def get_descriptions_date_details(response: list) -> dict:
    ...
    if response.css('meta[property="article:published_time"]').get():
        data_dict["published_at"] = response.css(
            'meta[property="article:published_time"]::attr(content)'
        ).get()
    else:
        pub_date = response.css("ul.info li::text").get()
...
def get_publisher_details(response) -> dict:
    ...
    """
    publisher = response.css('meta[property="dd:publisher"]::attr(content)').get()
    if publisher:
...
def get_text_title_section_tag_details(response: str) -> dict:
    ...
    return {
        "title": re.sub(
            r"[\r\t\n]",
            "",
            " ".join(response.css(".article-head strong::text").getall()),
        ).strip(),
        "text": [
            re.sub(
                r"[\r\t\n]", "", remove_tags(response.css("#articleBody").get())
            ).strip()
        ],
        "section": response.css(".subTitle::text").get(),
    }
...
def get_thumbnail_image_video(response: str) -> dict:
    ...
    thumbnail_url = []
    video = []
    thumbnail = response.css('meta[itemprop="thumbnailUrl"]::attr(content)').get()
    if thumbnail:
        thumbnail_url.append(thumbnail)
    for video_block in response.css("div.videoWrap video"):
        video_url = video_block.css("source::attr(src)").get()
        video.append(video_url)
    return remove_empty_elements(
        {
            "thumbnail_image": thumbnail_url,
            "embed_video_link": video,
        }
    )
