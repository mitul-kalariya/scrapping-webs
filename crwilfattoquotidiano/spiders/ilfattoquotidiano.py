...
class IlfattoquotidianoSpider(scrapy.Spider, BaseSpider):
    ...
    def __init__(self, *args, type=None, url=None, since=None, until=None, **kwargs):
        ...
        raw_cookie_data = get_request_headers()
        if raw_cookie_data[0]:
            data_block = raw_cookie_data[0]
            self.valid_cookie = data_block.get("name") + "=" + data_block.get("value")
            breakpoint()
            print(self.valid_cookie)
        self.header ={
            ...
            if self.type == "sitemap":
                if self.since and self.until:
                    ...
                    yield scrapy.Request(
                        response.url,
                        # headers=self.valid_request_headers,
                        # headers="cookie":"cf_clearance=HhYCzNteXtSHwLQ6uhGyxYAlrqCyX2VqKmFbwDrOVbU-1682056001-0-160",
                        cookies=self.valid_cookie,
                        # cookies={"cookie":"cf_clearance=HhYCzNteXtSHwLQ6uhGyxYAlrqCyX2VqKmFbwDrOVbU-1682056001-0-160"},
                        callback=self.parse_sitemap,
                        dont_filter=True,
                    )
