...
class TestArticle(unittest.TestCase):
    ...
    def test_parse(self):
        for article in TEST_ARTICLES:
            ...
            spider = MetropolesSpider(type="article", url=article["url"])
    ...
    def _test_parse_json_data_format(self, article, test_article_data):  # noqa: C901
        ...
        if article[0].get("parsed_data").get("images"):
            with self.subTest():
                self.assertIsInstance(article[0].get("parsed_data").get("images")[0],
                                      dict, "format mismatch for parsed_data--> images")
            with self.subTest():
